...
### 2.3 Create Store (Admin)
`POST /admin/stores`

### 2.4 Update Store (Admin)
`PUT /admin/stores/:storeId`

### 2.5 Toggle Store Status (Admin)
...
### 12.4 Delete / Archive Product
`DELETE /admin/products/:productId`

### 12.5 Upload Product Images
...
## 18. SEARCH & AUTO-SUGGEST (Customer)

### 18.1 Get Search Suggestions
`GET /search/suggestions?q=jack`
**Success Response**
```json
{
  "success": true,
  "data": {
    "suggestions": ["Jacket", "Jackets for men", "Leather jacket"],
    "popularSearches": ["Hoodies", "Sneakers"]
  }
}
```
...
### 19.1 Get Product Reviews (Customer)
`GET /products/:productId/reviews?page=1&limit=10`

### 19.2 Add Product Review (Customer)
...
```json
{
  ...
  "comment": "The fit is perfect and the material feels premium.",
  "images": ["url1", "url2"]
}
```

### 19.3 Manage Reviews (Admin)
...
```json
{
  ...
  "comments": "Need a size L instead of M."
}
```

### 20.2 Manage Returns (Admin)
- `GET /admin/returns`
- `PATCH /admin/returns/:id/status` (e.g., Approve, Reject, Refunded)

---

## 21. SUPPORT & TICKETING

### 21.1 Create Support Ticket (Customer)
`POST /support/tickets`
**Request Body**
```json
{
  "subject": "Where is my order?",
  "message": "It has been 3 days since the expected delivery.",
  "orderId": "ord_12345"
}
```

### 21.2 Manage Tickets (Admin)
- `GET /admin/tickets`
...
## 22. ANALYTICS & EXPORTS (Admin)

### 22.1 Export Orders (CSV)
`GET /admin/exports/orders?fromDate=2026-01-01&toDate=2026-07-01`
*(Returns a downloadable CSV file URL)*

### 22.2 Export Inventory (CSV)
`GET /admin/exports/inventory`
